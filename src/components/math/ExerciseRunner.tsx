...
export const ExerciseRunner: React.FC<{ def: ExerciseDef; onExit: () => void; onHome?: () => void; onNext?: () => void; nextTitle?: string }> = ({ def, onExit, onHome, onNext, nextTitle }) => {
  ...
  return <main className="recognition-game-screen mx-screen" dir="rtl">
    <GameHeader kicker={`${house.title} · ${lv.short}`} title={def.title} emoji={def.emoji} tone="sun" onBack={onExit} onHome={onHome}>
      <button type="button" className="kid-round-btn" onClick={() => { sound.playPop(); speakQ(); }} aria-label="دوباره بخوان"><Volume2 strokeWidth={2.8} /></button>
    </GameHeader>
    <div className="mx-body">
      ...
      {finished === null ? <>
        {def.type !== 'story' && <p className="mx-question">{round.question}</p>}
        <div className="mx-round" key={`${session}-${i}`}><R round={round} def={def} answer={answer} mistakes={mistakes} solved={solved} /></div>
        ...
        <OkArt className="mx-ok" ready caption="برگشت به خانه" onClick={() => { sound.playPop(); onExit(); }} />
      </section>}
    </div>
